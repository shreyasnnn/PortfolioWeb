...
export default function Menu({ items, children, scrollToSection }: MenuProps) {
  return (
    <div className="fixed bottom-4 sm:bottom-6 md:bottom-8 lg:bottom-10 left-1/2 -translate-x-1/2 z-50">
      <div className="flex gap-1 sm:gap-2 bg-white rounded-full shadow-lg p-1 sm:p-2">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="relative flex flex-col items-center group">
              ...
              <button
                ...
                className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 text-lg sm:text-xl transition-all duration-200 transform group-hover:-translate-y-2 sm:group-hover:-translate-y-3"
              >
                {item.logo}
              </button>
            </div>
